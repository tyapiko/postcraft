...
export default function PostsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-black mb-2">SNS投稿生成</h1>
            ...
            <RadioGroup value={platform} onValueChange={setPlatform}>
              {PLATFORMS.map((p) => (
                <div key={p.value} className="flex items-center space-x-2">
                  ...
                  <Label
                    htmlFor={p.value}
                    className={`cursor-pointer ${!p.available ? 'text-gray-400' : ''}`}
                  >
                    {p.label}
                    {!p.available && ' (近日対応予定)'}
                  </Label>
      ...
      {results.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-2xl font-bold text-black">生成結果</h2>
          {results.map((result) => (
            <Card key={result.pattern}>
              ...
              <CardContent className="space-y-4">
                <div className="p-4 rounded-lg bg-gray-50 whitespace-pre-wrap text-black">
                  {result.text}
                </div>
